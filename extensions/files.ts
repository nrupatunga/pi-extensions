	...
	pi.registerCommand("files", {
		...
		handler: async (_args, ctx) => {
			...
			const openSelected = async (file: FileEntry): Promise<void> => {
				try {
					// Open file in neovim in a tmux popup for inspection
					// Escape the file path for shell safety
					const escapedPath = file.path.replace(/'/g, "'\\''");
					
					// Use tmux display-popup to create a floating popup with neovim
					// -E: close popup when command exits
					// -w/-h: width/height as percentage
			...
			await ctx.ui.custom<void>((tui, theme, _kb, done) => {
				...
				selectList.onSelect = (item) => {
					void openSelected(item.value as FileEntry).then(() => done());
				};
